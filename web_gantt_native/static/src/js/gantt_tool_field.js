  ...
  function groupRows(tasks, group_bys, self_parent, ItemsSorted) {
    ...
    var split_groups = function (tasks, group_bys) {
      ...
      _.each(tasks, function (task) {
        ...
        var group = _.find(sp_groups, function (group) {
    ...
    if (second_sort) {
      ...
      groups = _.map(groups, function (result) {
        ...
        var sort_element = _.findWhere(second_sort, {
          id: s_id
        });
        if (sort_element) {
    ...
    if (main_group) {
      groups = _.map(groups, function (result) {
        var s_id = result['name'][0];
        var main_element = _.findWhere(main_group, {
          id: s_id
        });
        if (main_element) {
    ...
    var generate_task_info = function (task, plevel) {
      ...
      if (task.__is_group) {
        ...
        if (field && field.type === 'datetime') {
          ...
          group_name = field_utils.format[field.type](value_format, field, {
            timezone: false
          });
        } 
        else {
