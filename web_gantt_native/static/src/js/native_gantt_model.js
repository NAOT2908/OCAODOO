odoo.define('web_gantt_native.NativeGanttModel', function (require) {
  ...
  return AbstractModel.extend({
    ...
    on_data_loaded_predecessor: function (tasks, group_bys) {
      ...
      if (predecessor_model) {
        return this._rpc({
          ...
          fields: _.uniq([predecessor_task_id,
          predecessor_parent_task_id,
          predecessor_type])
        }).then(function (result) {
    ...
    get_minmax_step: function (tasks, group_bys) {
      ...
      self.gantt.data['GtimeStart'] = Math.min.apply(null, GtimeStartA);
      self.gantt.data['GtimeStop'] = Math.max.apply(null, GtimeStopA);
      GtimeStopA = [
      ];
      GtimeStartA = [
      ];
    ...
    get_res_load: function (tasks, group_bys) {
      ...
      if (self.LoadMode) {
        ...
        var data_load_group = _.where(self.gantt.data['rows_to_gantt'], {
          is_group: true
        });
