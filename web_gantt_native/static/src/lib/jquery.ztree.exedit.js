(function ($) {
  ...
  _bindEvent = function (setting) {
    ...
    o.bind(c.DRAG, function (event, srcEvent, treeId, treeNodes) {
      tools.apply(setting.callback.onDrag, [
        srcEvent,
        treeId,
        treeNodes
      ]);
    ...
    zTreeTools.copyNode = function (targetNode, node, moveType, isSilent) {
      ...
      if (moveType == consts.move.TYPE_INNER) {
        function copyCallback() {
          view.addNodes(_this.setting, targetNode, - 1, [
            newNode
          ], isSilent);
        }
        ...
        view.addNodes(this.setting, targetNode.parentNode, - 1, [
          newNode
        ], isSilent);
      ...
      function _docMouseMove(event) {
        ...
        if (root.dragFlag == 1) {
          ...
          if (tmpTarget && tmpTarget.attr('id') != targetSetting.treeId) {
            ...
            canInner = (isCopy || !isInner) && !(targetSetting.data.keep.leaf && !data.nodeIsParent(setting, tmpTargetNode)) && tools.apply(targetSetting.edit.drag.inner, [
              targetSetting.treeId,
              nodes,
              tmpTargetNode
                ...
                if (tmpTargetNode && data.nodeIsParent(setting, tmpTargetNode) && moveType == consts.move.TYPE_INNER) {
                  ...
                  if (startTimer) {
                    window.zTreeMoveTimer = setTimeout(function () {
                      ...
                      if (tmpTargetNode && data.nodeIsParent(setting, tmpTargetNode) && !tmpTargetNode.open && (new Date()).getTime() - startTime > targetSetting.edit.drag.autoOpenTime && tools.apply(targetSetting.callback.beforeDragOpen, [
                        targetSetting.treeId,
                        tmpTargetNode
                      ], true)) {
  ...
  _view = {
    ...
    addHoverDom: function (setting, node) {
      if (data.getRoots().showHoverDom) {
        ...
        tools.apply(setting.view.addHoverDom, [
          setting.treeId,
          node
        ]);
      }
    ...
    moveNode: function (setting, targetNode, node, moveType, animateFlag, isSilent) {
      ...
      if (!nodeDom.get(0)) {
        nodeDom = view.appendNodes(setting, node.level, [
          node
        ], null, - 1, false, true).join('');
    ...
    removeTreeDom: function (setting, node) {
      ...
      tools.apply(setting.view.removeHoverDom, [
        setting.treeId,
        node
      ]);
    },
