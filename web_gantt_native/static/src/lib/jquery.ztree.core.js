(function ($) {
  ...
  _bindEvent = function (setting) {
    ...
    o.bind(c.NODECREATED, function (event, treeId, node) {
      tools.apply(setting.callback.onNodeCreated, [
        event,
        treeId,
        node
      ]);
    ...
    o.bind(c.COLLAPSE, function (event, treeId, node) {
      tools.apply(setting.callback.onCollapse, [
        event,
        treeId,
        node
      ]);
    ...
    o.bind(c.REMOVE, function (event, treeId, treeNode) {
      tools.apply(setting.callback.onRemove, [
        event,
        treeId,
        treeNode
      ]);
    ...
    o.bind(c.SELECTED, function (event, treeId, node) {
      tools.apply(setting.callback.onSelected, [
        treeId,
        node
      ]);
    });
    o.bind(c.UNSELECTED, function (event, treeId, node) {
      tools.apply(setting.callback.onUnSelected, [
        treeId,
        node
      ]);
    });
  ...
  view = {
      ...
      $.ajax({
        ...
        success: function (msg) {
          ...
          if (newNodes && newNodes !== '') {
            ...
            view.addNodes(setting, node, - 1, !!newNodes ? tools.clone(newNodes) : [
            ], !!isSilent);
          } else {
    ...
    createNodeCallback: function (setting) {
      if (!!setting.callback.onNodeCreated || !!setting.view.addDiyDom) {
        ...
        while (root.createdNodes.length > 0) {
          ...
          tools.apply(setting.view.addDiyDom, [
            setting.treeId,
            node
          ]);
    ...
    makeNodeClasses: function (setting, node) {
      var classes = tools.apply(setting.view.nodeClasses, [
        setting.treeId,
        node
  ...
  $.fn.zTree = {
    ...
    init: function (obj, zSetting, zNodes) {
      ...
      data.initRoot(setting);
      var root = data.getRoot(setting);
      zNodes = zNodes ? tools.clone(tools.isArray(zNodes) ? zNodes : [
        zNodes
      ]) : [
      ];
      if (setting.data.simpleData.enable) {
      ...
      var zTreeTools = {
        ...
        addNodes: function (parentNode, index, newNodes, isSilent) {
          ...
          var xNewNodes = tools.clone(tools.isArray(newNodes) ? newNodes : [
            newNodes
          ]);
