...
class StockPicking(models.Model):
    ...
    def get_xlsx_report(self, datas, response):
        ...
        for picking in self.env['stock.picking'].browse(datas):
            ...
            txt = workbook.add_format({'font_size': 12, 'bold': True})
            date = workbook.add_format(
            {'font_size': 12, 'num_format': 'yyyy-m-d', 'bold': True})
            txt_border = workbook.add_format(
                {'font_size': 15, 'align': 'center', 'valign': 'vcenter', 'bold': True, 'border': 1, 'bg_color': '#02affe'})
            # txt_border.set_bg_color('#02affe')
            ...
            sheet.write('D6', 'Scheduled Date', date)
            sheet.write('D7', 'Effective Date', date)
            ...
            sheet.write('B14', ref)
            sheet.merge_range('A16:A17', 'Product', txt_border)
            sheet.merge_range('B16:B17', 'Description', txt_border)
            sheet.merge_range('C16:C17', 'Scheduled Date', txt_border)
            sheet.merge_range('D16:D17', 'Deadline', txt_border)
            sheet.merge_range('E16:E17', 'Quantity', txt_border)
            sheet.merge_range('F16:F17', 'Quantity Done', txt_border)
            sheet.merge_range('G16:G17', 'Note', txt_border)
            row = 18
    ...
    def _add_picking_line_to_excel(self, sheet, picking, row, border):
        ...
        for line in picking.move_ids:
            ...
            sheet.write(row, 6, line.note_stock, border)
            row += 1
