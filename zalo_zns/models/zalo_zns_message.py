...
class ZaloZnsMessage(models.Model):
    ...
    #     return phone
    def format_phone_number(self, phone):
        # Kiểm tra nếu phone là None hoặc không phải là chuỗi
        if not phone or not isinstance(phone, str):
            return ''  # Trả về chuỗi rỗng nếu phone không hợp lệ
        
        # Loại bỏ các ký tự không phải số
        phone = re.sub(r'\D', '', phone)
        
        # Kiểm tra nếu số điện thoại bắt đầu với '0' và thay bằng '84'
        if phone.startswith('0'):
            phone = '84' + phone[1:]
        
        # Nếu số điện thoại không bắt đầu với '84', thêm '84'
        if not phone.startswith('84'):
            phone = '84' + phone
