...
import 'package:inven_barcode_app/commons/time_debouncer.dart';
import 'package:inven_barcode_app/widets/form/form_select_live_search/dart/form_select_live_search_controller.dart';

class FormSelectLiveSearch<T extends Object> extends StatefulWidget {
  ...
  // Search include when value enter is empty
  ...
  @override
  State<FormSelectLiveSearch<T>> createState() => _SearchableDropdownState<T>();
}

class _SearchableDropdownState<T extends Object>
    extends State<FormSelectLiveSearch<T>> {
  late FormSelectLiveSearchController<T> controller;
  final _searchTimeDeBouncer = TimeDeBouncer(milliseconds: 750);

  @override
  void initState() {
    controller = FormSelectLiveSearchController<T>();
        ...
        return TextField(
          ...
          onTap: () {
            if (textEditingController.text.trim().isEmpty &&
                widget.searchWhenEmpty == false) return;
  ...
  Future<List<T>> optionsBuilder(TextEditingValue search) async {
    if ((widget.searchWhenEmpty == false || search.text.trim().isNotEmpty) &&
        controller.searchText.value.trim() == search.text.trim()) {
